...
class RequestContextMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        is_api_route = request.url.path.startswith(
            "/api/"
        ) or request.url.path.startswith("/v1/")
        is_public_endpoint = not is_api_route or request.url.path.startswith(
            "/api/config"
        )
        ...
        if not is_public_endpoint:
            if not tenant_id or tenant_id == "unknown":
                ...
                return JSONResponse(
                    status_code=400,
                    content={
                        "error": "tenant_id header is strictly required for isolation",
        ...
        if settings.API_KEY and not is_public_endpoint:
            ...
            if client_key != settings.API_KEY:
                logger.warning(
                    ...
                    extra={"event": "UNAUTHORIZED_ACCESS"},
                )
                trace_id_var.reset(token_t)
                span_id_var.reset(token_s)
                tenant_id_var.reset(token_ten)
                return JSONResponse(
                    status_code=401,
                    content={
                        "detail": "Invalid or missing API Key. This is a secured standalone instance."
                    },
                )
