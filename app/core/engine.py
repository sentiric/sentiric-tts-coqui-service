...
from app.core.normalizer import normalizer  # YENİ
from app.core.audio import audio_processor  # YENİ
...
class TTSEngine:
    ...
    # SSML Parser hala burada çünkü model inference ile doğrudan ilgili
    def _parse_ssml(self, ssml_text: str):
        # ... (Eski kodun aynısı, kısaltıldı) ...
        try:
            ...
            def process_element(element, current_params):
                ...
                for child in element:
                    ...
                    if child.tag == 'break':
                        ...
                        if rate in mapping: child_params['speed'] = mapping[rate]
                        else: 
                            try: child_params['speed'] = float(rate)
                            except: pass
    ...
    def synthesize(self, params: dict, speaker_wavs=None) -> bytes:
        ...
        # 2. Cache Check (Cache logic engine'de kalabilir veya ayrılabilir, şimdilik burada)
        cache_key = None
        if not is_ssml and not speaker_wavs:
            cache_key = self._generate_cache_key({**params, "text": text}) # Normalize edilmiş text ile key üret
        ...
        # 3. Audio Processing (SRP - Delegated)
        ...
        final_audio = audio_processor.process_audio(
            ...
            add_silence=True # Non-stream için sessizlik ekle
        )
    ...
    # ... (refresh_speakers, get_speakers, _get_latents, _generate_cache_key, _check_cache, _save_cache) ...
    # Bu metodlar logic gereği burada kalabilir çünkü model yönetimiyle ilgililer.
    # Ancak FFmpeg kodlarını audio_processor'a taşıdığımız için _process_audio metodunu buradan SİLİYORUZ.
    
    # (Kısalık adına diğer metodların mevcut hali korundu varsayalım)
