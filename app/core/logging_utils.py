# Dosya: app/core/logging_utils.py
import logging
import sys
import os
import contextvars
...
# [ARCH-COMPLIANCE] Asenkron Context Propagation (trace_id, span_id, tenant_id)
...
class CustomJsonFormatter(jsonlogger.JsonFormatter):
    """[ARCH-COMPLIANCE] Platform standardı SUTS v4.0 ile uyumlu JSON log formatı."""
    def add_fields(self, log_record, record, message_dict):
        super(CustomJsonFormatter, self).add_fields(log_record, record, message_dict)
        
        # Varsayılan pythonjsonlogger alanlarını temizleyip sıfırdan inşa et
        for key in list(log_record.keys()):
            if key != 'message':
                del log_record[key]

        log_record['schema_v'] = "1.0.0"
        # ISO 8601 UTC Formatı (ts)
        ...
        # Context'ten ID'leri çek, yoksa log argümanlarından (extra) al
...
def setup_logging():
    ...
    logger.info("Log system initialized in PRODUCTION (SUTS v4.0 JSON) mode.", extra={"event": "LOGGER_INIT"})