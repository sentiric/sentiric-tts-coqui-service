...
class Settings(BaseSettings):
    ...
    # --- TLS / mTLS CONFIG ---
    GRPC_TLS_CA_PATH: str = os.getenv(
        "GRPC_TLS_CA_PATH", "/sentiric-certificates/certs/ca.crt"
    )
    TTS_COQUI_SERVICE_CERT_PATH: str = os.getenv(
        "TTS_COQUI_SERVICE_CERT_PATH",
        "/sentiric-certificates/certs/tts-coqui-service.crt",
    )
    TTS_COQUI_SERVICE_KEY_PATH: str = os.getenv(
        "TTS_COQUI_SERVICE_KEY_PATH",
        "/sentiric-certificates/certs/tts-coqui-service.key",
    )
    ...
    LOW_RESOURCE_MODE: bool = (
        os.getenv("TTS_COQUI_SERVICE_LOW_RESOURCE_MODE", "true").lower() == "true"
    )

    # --- TORCH SETTINGS ---
    ENABLE_DEEPSPEED: bool = (
        os.getenv("TTS_COQUI_SERVICE_ENABLE_DEEPSPEED", "false").lower() == "true"
    )
    ENABLE_HALF_PRECISION: bool = (
        os.getenv("TTS_COQUI_SERVICE_ENABLE_HALF_PRECISION", "true").lower() == "true"
    )
    ENABLE_ATTENTION_KV_CPU_OFFLOAD: bool = (
        os.getenv("TTS_COQUI_SERVICE_ENABLE_ATTENTION_KV_CPU_OFFLOAD", "true").lower()
        == "true"
    )
    ENABLE_STREAMING: bool = (
        os.getenv("TTS_COQUI_SERVICE_ENABLE_STREAMING", "false").lower() == "true"
    )
    ...
    DEFAULT_TEMPERATURE: float = float(
        os.getenv("TTS_COQUI_SERVICE_DEFAULT_TEMPERATURE", "0.75")
    )
    ...
    DEFAULT_REPETITION_PENALTY: float = float(
        os.getenv("TTS_COQUI_SERVICE_DEFAULT_REPETITION_PENALTY", "2.0")
    )

    DEFAULT_OUTPUT_FORMAT: str = os.getenv(
        "TTS_COQUI_SERVICE_DEFAULT_OUTPUT_FORMAT", "wav"
    )
    DEFAULT_SAMPLE_RATE: int = int(
        os.getenv("TTS_COQUI_SERVICE_DEFAULT_SAMPLE_RATE", "24000")
    )
