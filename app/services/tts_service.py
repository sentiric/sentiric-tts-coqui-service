...
async def download_temp_wav(url: str) -> str:
    if not is_allowed_speaker_url(url):
        logger.warn("SSRF_ATTEMPT: İzin verilmeyen bir speaker URL'i engellendi.", url=url)
    ...
    async with httpx.AsyncClient() as client:
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=".wav") as tmp_file:
            ...
            logger.debug("Dinamik referans sesi başarıyla indirildi.", url=url, path=tmp_file.name)
...
class CoquiTTSEngine:
    ...
    def _get_device(self) -> str:
        ...
        if device_setting == "auto":
            ...
            self.logger.info(f"CUDA available: {is_cuda}")
    ...
    async def synthesize(self, text: str, language: str, speaker_wav_path: Optional[str] = None) -> bytes:
        ...
        try:
            ...
            def _synthesize_sync():
                self.logger.debug("Coqui XTTS ile sentezleme başlıyor...", speaker=speaker_ref_path)
                ...
                wav_buffer.seek(0)
                self.logger.debug("Coqui XTTS ile sentezleme tamamlandı.")
